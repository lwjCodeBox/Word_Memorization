...
CGridCtrl::CGridCtrl(int nRows, int nCols, int nFixedRows, int nFixedCols)
{
    ...
    m_bTrackFocusCell     = TRUE;       // LWJ - true이면 클릭 한 셀이 몇행몇열인지 고정된 픽셀로 알려줌.
...
BOOL CGridCtrl::RedrawCell(int nRow, int nCol, CDC* pDC /* = NULL */)
{
    ...
    // LWJ : if this is a merge cell then we have to make sure there are no drawing problem becoz of direct redraw of cell
